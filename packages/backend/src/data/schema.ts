...
export const repository = sqliteTable('repository', {
  ...
  index('idx_repo_project').on(table.projectId),
]);
...
// ─── member_identity ─────────────────────────────
export const memberIdentity = sqliteTable('member_identity', {
  id: text('id').primaryKey(),
...
// ─── code_change ─────────────────────────────────
export const codeChange = sqliteTable('code_change', {
  id: text('id').primaryKey(),
  projectId: text('project_id').notNull().references(() => project.id),
  repoId: text('repo_id').notNull().references(() => repository.id),
  type: text('type').notNull(),
...
export const deepAnalysis = sqliteTable('deep_analysis', {
  id: text('id').primaryKey(),
  codeChangeId: text('code_change_id').notNull().references(() => codeChange.id).unique(),
...
// ─── evaluation_entry ────────────────────────────
export const evaluationEntry = sqliteTable('evaluation_entry', {
  id: text('id').primaryKey(),
  memberId: text('member_id').notNull().references(() => member.id),
  type: text('type').notNull(),
...
export const scanState = sqliteTable('scan_state', {
  id: text('id').primaryKey(),
  repoId: text('repo_id').notNull().references(() => repository.id).unique(),
